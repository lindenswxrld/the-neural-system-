...
export default function NeuralSystem() {
  ...
  const toggleCompare = async (audit: any) => {
    if (compareList.find(a => a.id === audit.id)) {
      const newList = compareList.filter(a => a.id !== audit.id);
      ...
      alert("Select 2 candidates for comparison.");
    }
  };
  ...
  return (
    <div className="min-h-screen bg-black text-zinc-100 p-4 md:p-12">
      <div className="max-w-7xl mx-auto space-y-8">
        <header className="flex justify-between items-end border-b border-zinc-900 pb-8 print:hidden">
          <div className="text-left space-y-2">
            ...
            <p className="text-zinc-600 text-[9px] uppercase font-bold tracking-widest italic">v1.9.5 // Comparative Intelligence</p>
          </div>
          <Button onClick={() => window.print()} variant="outline" className="border-zinc-800 text-zinc-500 text-[10px] font-bold uppercase">Export PDF</Button>
        </header>
          ...
          <div className="lg:col-span-3 space-y-6 print:hidden">
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <input type="text" value={candidateName} onChange={(e) => setCandidateName(e.target.value)} className="w-full bg-zinc-950 border border-zinc-900 rounded-lg p-3 text-xs text-white outline-none" placeholder="Enter Name..." />
              </div>
              <div className="space-y-2">
                ...
                <textarea value={text} onChange={(e) => setText(e.target.value)} className="w-full h-32 bg-zinc-950 border border-zinc-900 rounded-xl p-4 text-xs text-zinc-400 outline-none" placeholder="Paste Data..." />
              </div>
              <Button onClick={handleAnalyze} disabled={isAnalyzing} className="w-full bg-white text-black font-black text-xs uppercase">{isAnalyzing ? <Loader2 className="animate-spin w-4 h-4" /> : "Initiate Audit"}</Button>
            </div>
            
            <div className="pt-6 border-t border-zinc-900">
              <h3 className="text-[10px] font-black text-zinc-600 uppercase mb-4">Audit Registry</h3>
              <div className="space-y-2">
                {history?.map((item) => (
                  <div key={item.id} className="flex gap-2">
                    <button onClick={() => {setCurrentAudit(item); setCompareList([]);}} className={`flex-1 text-left p-3 rounded-xl border text-[10px] font-bold uppercase ${currentAudit?.id === item.id ? 'border-violet-500 bg-violet-500/5' : 'border-zinc-900'}`}>{item.name}</button>
          ...
          <div className="lg:col-span-9 space-y-12">
            {compareList.length === 2 ? (
              ...
              <div className="space-y-12 animate-in fade-in duration-700">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center bg-zinc-950/30 p-8 rounded-3xl border border-zinc-900">
                  ...
                  <div className="space-y-4">
                     <h3 className="text-[10px] font-black uppercase text-violet-500 italic">{currentAudit.name} Signature</h3>
                ...
                <div className="bg-zinc-950/30 p-8 rounded-3xl border border-rose-900/20 space-y-6">
                  <h3 className="text-[10px] font-black uppercase text-rose-500 flex items-center gap-2"><AlertTriangle className="w-3 h-3"/> Toxicity Audit</h3>
                  <div className="grid grid-cols-3 gap-6">
                    {currentAudit.darkTriad?.map((trait: any) => (
                      <div key={trait.name} className="space-y-2">
                        <div className="flex justify-between text-[9px] uppercase font-bold text-zinc-500"><span>{trait.name}</span><span className="text-white">{trait.score}%</span></div>
                        <div className="h-[2px] w-full bg-zinc-900 rounded-full"><div className="h-full bg-rose-600" style={{ width: `${trait.score}%` }} /></div>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            {history.length > 0 && (
              <div className="space-y-6">
                <h3 className="text-[10px] font-black uppercase text-zinc-500 flex items-center gap-2"><TrendingUp className="w-3 h-3"/> Institutional Ranking</h3>
                <div className="bg-zinc-950 border border-zinc-900 rounded-2xl overflow-hidden text-[10px]">
                  <table className="w-full text-left">
                    <thead className="bg-zinc-900/50 text-zinc-500 uppercase font-black"><tr><th className="p-4">Candidate</th><th className="p-4 text-center">Score</th><th className="p-4">Risk</th><th className="p-4 text-right">Action</th></tr></thead>
                    <tbody className="text-white">
                      {[...history].sort((a,b) => b.score - a.score).map((item) => (
                        <tr key={item.id} className="border-b border-zinc-900/50 hover:bg-zinc-900/20">
                          <td className="p-4 font-black italic uppercase">{item.name}</td>
                          <td className="p-4 text-center font-mono">{item.score}</td>
                          <td className="p-4">{item.darkTriad.some((t: any) => t.score > 60) ? <span className="text-rose-500 font-black">High Risk</span> : <span className="text-zinc-600">Stable</span>}</td>
                          <td className="p-4 text-right"><button onClick={() => setCurrentAudit(item)} className="text-violet-500 hover:text-white uppercase font-black">Review</button></td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
